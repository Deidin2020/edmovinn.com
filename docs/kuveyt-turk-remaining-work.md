...
`.env` يشير إلى `sanalpos.kuveytturk.com.tr` (الإنتاج) مباشرة ولا يوجد إعداد
اختبار. المطلوب من البنك: روابط بيئة الاختبار، بيانات اعتماد اختبارية، وبطاقات
اختبار. لا يُختبر المسار بعملية حقيقية على الإنتاج.
